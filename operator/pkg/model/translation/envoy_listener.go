...
func (i *cecTranslator) httpsFilterChains(name string, m *model.Model) ([]*envoy_config_listener.FilterChain, error) {
	tlsToHostnamesWithValidation := m.TLSSecretsToHostnamesWithValidation()
	if len(tlsToHostnamesWithValidation) == 0 {
		return nil, nil
	}

	var filterChains []*envoy_config_listener.FilterChain

	orderedSecrets := goslices.SortedStableFunc(maps.Keys(tlsToHostnamesWithValidation), func(a, b model.TLSSecret) int {
		return cmp.Compare(a.Namespace+"/"+a.Name, b.Namespace+"/"+b.Name)
	})

	for _, secret := range orderedSecrets {
		data := tlsToHostnamesWithValidation[secret]
